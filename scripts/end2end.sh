...
if [ "$MODE" = "train" ]
then
  python main.py --model=end2end --mode=train --data_path=$TRAIN_PATH --vocab_path=$VOCAB_PATH --log_root=log \
   --exp_name=$EXP_NAME --max_enc_steps=$MAX_ENC_STEPS --max_dec_steps=100 --max_train_iter=$MAX_ITER \
   --batch_size=$BATCH_SIZE --max_art_len=$MAX_ART_LEN --lr=$LR --selector_loss_wt=$SELECTOR_LOSS_WT \
   --inconsistent_loss=$INCONSISTENT_LOSS --inconsistent_topk=$INCONSISTENT_TOPK --save_model_every=$SAVE_MODEL_EVERY \
   --model_max_to_keep=$MAX_TO_KEEP --coverage=True \
   # --pretrained_selector_path=$SELECTOR_PATH --pretrained_rewriter_path=$REWRITER_PATH
elif [ "$MODE" = "eval" ]
then
